...
We'll also look at a joy plot, because Dr. Love enjoys them, even though they're really more useful when we're comparing more than two samples.

```{r blead_tidied_joyplot}
ggplot(blead_tidied, aes(x = leadcontent, y = status, fill = status)) +
    ggjoy::geom_joy(scale = 0.9) +
    guides(fill = FALSE) + 
    labs(title = "Joy Plot of Lead Content in Exposed and Control kids") +
    ggjoy::theme_joy()
```
