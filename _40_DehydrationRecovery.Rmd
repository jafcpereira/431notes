...
First, we'll check ranges (and for missing data) in the `hydrate` file. Note that Gini's mean difference (`Gmd` in the output below) is a robust measure of spread that describes the mean absolute difference between any pair of observations. 

```{r p3}
hydrate
Hmisc::describe(hydrate)
```
...
```{r scatterplot matrix of hydrate data with ggpairs}
...
```{r eda1sam for recov.score}
eda.1sam(dataframe = hydrate, variable = hydrate$recov.score, 
         x.title = "Recovery Score", 
         ov.title = "Recovery Scores from 36 children in the hydrate study")
```
...
```{r simple regression scatterplot}
ggplot(hydrate, aes(x = dose, y = recov.score)) +
    geom_point(size = 2) +
    geom_smooth(method = "lm", col = "red") +
    labs("Simple Regression model for the hydrate data")
```
...
### Model Coefficients: The Rest of the Output
...
Next to each coefficient in the table above is its estimated standard error, followed by the coefficient's t value (the coefficient value divided by the standard error), and the associated two-tailed *p* value for the test of:
...
```{r resid plot for hydrate simple regression}
plot(lm(recov.score ~ dose, data = hydrate), which = 1)
```

As mentioned previously, we are looking in this plot for a generally random scatter of points, perhaps looking like a "fuzzy football". Since we only have seven possible `dose` values, we obtain only seven distinct predicted values, which explains the seven vertical lines in the plot. Here, the smooth red line indicates a gentle curve, but no evidence of a strong curve, or any other regular pattern in this residual plot.

### Saving Residuals and Fitted Values

To save the residuals and predicted (fitted) values from this simple regression model, we can use the `resid` and `fitted` commands, respectively. 
