...
b.	Consider re-expressing the data (often we start with re-expressions of the outcome, or Y, data) using a transformation so that the transformed data may be modeled effectively using a straight line. 
...
The **Box-Cox plot**, from the `boxCox` function in the `MASS` package, sifts through the ladder of options to suggest a transformation (for Y) to best linearize the outcome-predictor(s) relationship.
...
Here, we use the `testTransform` function to compare the optimal choice determined by the powerTransform function (here $\lambda$ = 0.44) to $\lambda$ = 0 (logarithm), 0.5 (square root) and 1 (no transformation). 
