...
```{r histograms for battery, fig.height = 3.5}
...
```{r paired diffs breakfast, fig.height=4, echo = FALSE}
breakfast$diffs <- breakfast$cornflakes - breakfast$oatbran

p1 <- ggplot(breakfast, aes(x = diffs)) +
  geom_histogram(aes(y = ..density..), bins = fd_bins(breakfast$diffs),
                 fill = "steelblue", col = "white") +
  stat_function(fun = dnorm, lwd = 1.5, col = "red",
                args = list(mean = mean(breakfast$diffs), 
                            sd = sd(breakfast$diffs))) +
  coord_flip() +
  labs(title = "Histogram",
       x = "Difference in LDL", y = "Density")

p2 <- ggplot(breakfast, aes(x = 1, y = diffs)) +
  geom_boxplot(fill = "steelblue") +
  theme(axis.text.x = element_blank(), axis.ticks.x = element_blank()) +
  labs(title = "Boxplot",
       y = "", x = "")

p3 <- ggplot(breakfast, aes(sample = diffs)) +
  geom_qq(col = "steelblue", size = 2) +
  geom_abline(intercept = qq_int(breakfast$diffs), 
              slope = qq_slope(breakfast$diffs)) +
  labs(title = "Normal Q-Q",
       y = "", x = "")

gridExtra::grid.arrange(p1, p2, p3, nrow=1, 
   top = "Difference in LDL (Corn Flakes - Oat Bran)")
```
...
Based on this sample of 14 subjects in the crossover study, we reject H~0~ here and conclude that there is a statistically significant (detectable) difference between the LDL cholesterol levels after eating corn flakes and eating oat bran, at a 5% significance level.
