...
> ... if you know the data are paired, you can take advantgage of that fact - in fact, you *must* take advantage of it. ... You must desicde whether the data are paired from understanding how they were collected and what they mean. ... There is no test to determine whether the data are paired.
...
We'll also look at a joy plot.

```{r sepsis_temp_drop_by_treat_joyplot}
ggplot(sepsis, aes(x = temp_drop, y = treat, fill = treat)) +
    ggjoy::geom_joy(scale = 0.9) +
    guides(fill = FALSE) + 
    labs(title = "Joy Plot of Temperature Drop in Sepsis Patients",
