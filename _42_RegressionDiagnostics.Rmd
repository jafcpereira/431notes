...
Multiple linear regression (also called ordinary least squares, or OLS) has four main assumptions\footnote{Some of this discussion is adapted from the Multiple Regression Supplement of Bock, Velleman and DeVeaux' Stats text.} that are derived from its model.
...
Unfortunately, assuming the model is true is not sufficient to prove that the linear model fits, so we check what Bock, Velleman and Deveaux call the "Straight Enough Condition" 
...
```{r p26, echo=FALSE, fig.height=3.5}
...
```{r p31 constant vs nonconstant variance examples, fig.height=7, echo=FALSE}
set.seed(5)
...
- My favorite is model diagnostic plot 5 – the residuals vs. leverage plot, which has contours to indicate the value of Cook’s distance. 
...
```{r cooks2, fig.height=3.5, fig.width=5, fig.align="center"}
...
- This observation was included in the sample in error – the subject was not eligible.
...
```{r boxCox for outcome in violsim1}
...
```{r violsim 1 model 2 scale location plot}
...
```{r fit model 2 without point 72 plots 1 and 3}
...
```{r fit model 2 without point 72 plots 2 and 5}
par(mfrow=c(1,2))
