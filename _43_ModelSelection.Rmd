...
We’ll spend considerable time in 432 studying how best to validate a regression model, but for now, I’ll focus on a pair of issues:
...
To address these issues, I’ll look at one of our old examples: the `wcgs` data (Western Collaborative Group Study), where I’ll try to predict the variable `chol` on the basis of some subset of the following five predictors: `age`, `bmi`, `sbp`, `dbp` and `smoke`.
...
2.	Using only the model development sample, run a stepwise regression algorithm working off of the kitchen sink model using backwards selection to identify a reasonable candidate for a model. We’ll call this model A.
3.	Develop a second potential model (which we’ll call model B) for the model development data by eliminating the least clearly helpful predictor in model A.
